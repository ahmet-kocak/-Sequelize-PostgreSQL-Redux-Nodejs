...
extraReducers: {
  ...
  [getList.pending]: (state, action) => {state.isFetching = true},
  ...
  [getList.rejected]: (state, action) => { 
    state.isFetching = false; state.error= true; state.loading=false ;},

  
  [postNewRecord.pending]: (state, action) => {state.isFetching = true;
    state.error=false},

  [postNewRecord.fulfilled]: (state, action) => {
    state.message="Registration Successful";
    state.error=false
    state.loading=true ; state.isFetching = false},

  [postNewRecord.rejected]: (state, action) => {
    state.message="Registration failed";

    state.isFetching = false; state.error= true; state.loading=false }, 


  [updateSort.pending]: (state, action) => {state.isFetching = true},

  [updateSort.fulfilled]: (state, action) => {
    state.loading=true ; state.isFetching = false;
    state.bookList=action.payload.data},

  [updateSort.rejected]: (state, action) => {
    state.isFetching = false; state.error= true; state.loading=false }, 

 
    [getSelect.pending]: (state, action) => {state.isFetching = true},

    [getSelect.fulfilled]: (state, action) => {
      state.loading=true ; state.isFetching = false;
      state.selectList=action.payload.data},

    [getSelect.rejected]: (state, action) => {
      state.isFetching = false; state.error= true; state.loading=false },
